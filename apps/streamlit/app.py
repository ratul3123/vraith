...
# Importing raw service primitives for granular live step-by-step sidebar tracking
...
from vraith.services.rag.rag_engine import build_rag_chain, ask_questions

load_dotenv()

# ─── Page Config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Vraith - Intelligent Video Assistant",
    page_icon="🎬",
    layout="wide",
...
# ─── Sidebar ────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div class="hero-title" style="font-size:1.6rem">🎬 V R A I T H</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_done or st.session_state.processing:
        st.markdown("---")
        st.markdown('<span class="badge badge-green">Pipeline Tracker</span>', unsafe_allow_html=True)
        for step, icon, label in [
            ("audio",      "🔊", "Decomposing Audio Source"),
            ("transcript", "📝", "Neural Transcription"),
            ("title",      "🏷️", "Synthesizing Context Title"),
            ("summary",    "📋", "Abstractive Summarization"),
            ("extract",    "🔍", "Primitive Extraction"),
            ("rag",        "🧠", "RAG Vector Architecture"),
        ]:
            render_step_bar(label, step, icon)

# ─── Main Area ──────────────────────────────────────────────────────────────────
st.markdown('<div class="hero-title">Vraith - Workspace</div>', unsafe_allow_html=True)
    ...
    if not source:
        ...
        try:
            with progress_placeholder.container():
                st.info("⚙️ Initializing Pipeline Primitives — check sidebar map for updates.")

            update_step("audio", "active")
            st.sidebar.empty() # Forces sidebar re-rendering context
...
if st.session_state.result:
    ...
    if st.session_state.chat_history:
        ...
        for msg in st.session_state.chat_history:
            if msg["role"] == "user":
                ...
                <div class="chat-msg" style="align-items:flex-start">
                    <span class="chat-label bot-label">🤖 Vraith Assistant</span>
        ...
        <div class="card" style="text-align:center;padding:2rem">
            ...
            <div style="color:var(--text-muted);font-size:0.85rem">Ask anything about your video</div>
        </div>""", unsafe_allow_html=True)
    ...
    if send_btn and user_input.strip():
        with st.spinner("Thinking…"):
            answer = ask_questions(r["rag_chain"], user_input.strip())
        st.session_state.chat_history.append({"role": "user",      "content": user_input.strip()})
        st.session_state.chat_history.append({"role": "assistant", "content": answer})
        st.rerun()

    if st.session_state.chat_history:
        if st.button("🗑️ Clear Chat", key="clear_chat_history_button"):
            st.session_state.chat_history = []
            st.rerun()
